...
public:
    template<typename Func>
    TransactLogHandler(RealmDelegate* delegate, Func&& func)
    : m_delegate(delegate)
    {
        if (!delegate) {
            func();
    ...
    bool link_list_clear(size_t old_size) {
        ...
        o->indices.resize(old_size);
        std::iota(begin(o->indices), end(o->indices), 0);
...
void Realm::begin_transaction()
{
    ...
    TransactLogHandler(m_delegate.get(), [&](auto&&... args) {
